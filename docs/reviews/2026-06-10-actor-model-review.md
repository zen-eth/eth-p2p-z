...
| 3 | P2 sendmmsg + GSO + flushScheduled | M | **P2-a GSO done** (`946c473`: one sendmsg per flush burst via UDP_SEGMENT cmsg; MEASURED Linux fast-mode bulk ~1.1→3.4 Gbit/s, small ~1.1→3.7 (~3×); full matrix green incl. transport-interop 21/21 + testground 3/3 + both Shadows with GSO active where the kernel allows; review caught 4 pre-commit: per-packet pktinfo capture, downgrade error discrimination, 65535-byte cap, post-send stats). P2-b sendmmsg parked (neither quinn nor quic-go bothers). P2-c (zio inline try-send for small-packet fan-out) open — upstream zio candidate. Earlier note: **GSO needed NO zio change** — zio's send path is real sendmsg with cmsg passthrough (`netSendImpl` forwards `OutgoingMessage.control`; verified down to `os.net.sendmsg(..., control)` in both backends), and our `socket_control` already builds outgoing cmsgs (PKTINFO), so P2-a = coalesce a flush burst's equal-sized packets + one `UDP_SEGMENT` cmsg, our side only, Linux, runtime-probed like UDP_GRO. Only P2-b (sendmmsg: cross-destination batching) needs a new zio op — and neither quinn nor quic-go bothers (both ship per-connection GSO only), so it stays parked. NEW P2-c (small-packet fan-out lever, attestation shape): zio's epoll send path is poll()+sendmsg()+fiber round trip PER packet (ev/blocking.zig:448-465) — an inline MSG_DONTWAIT try-send that parks only on EAGAIN would cut it to one syscall; upstream zio candidate, orthogonal to GSO. Order: P2-a, re-measure with P0, then judge P2-c. |
