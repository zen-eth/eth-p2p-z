...
pub fn generateProtobuf(
    ...
    var parsed = try gremlin_parser.parse(allocator, proto_root, io);
    defer parsed.deinit();
    ...
    try generateCode(allocator, &files, io);
}
...
fn generateCode(allocator: std.mem.Allocator, files: *std.ArrayList(ZigFile), io: std.Io) !void {
    for (files.items) |*file| {
