...
pub const ParserBuffer = struct {
    ...
    pub fn initFile(allocator: std.mem.Allocator, path: []const u8, io: std.Io) !ParserBuffer {
        const file = try std.Io.Dir.openFileAbsolute(io, path, .{ .mode = .read_only });
        defer file.close(io);

        const stat_result = try file.stat(io);
        const file_size: usize = @intCast(stat_result.size);
        const buf = try allocator.alloc(u8, file_size);
        errdefer allocator.free(buf);

        const bytes_read = try file.readPositionalAll(io, buf, 0);
        return ParserBuffer{ .buf = buf[0..bytes_read], .allocator = allocator };
    }
