...
pub fn parse(allocator: std.mem.Allocator, base_path: []const u8, io: std.Io) !ParseResult {
    // Find all .proto files and their common root
    var proto_files = try paths.findProtoFiles(allocator, base_path, io);
    defer {
    ...
    const root = if (!std.fs.path.isAbsolute(base_path))
        (try std.Io.Dir.cwd().realPathFileAlloc(io, base_path, allocator))[0..]
    else
    ...
    for (proto_files.items) |file_path| {
        var buffer = try ParserBuffer.initFile(allocator, file_path, io);
