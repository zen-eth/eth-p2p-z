...
pub fn findProtoFiles(allocator: std.mem.Allocator, basePath: []const u8, io: std.Io) !std.ArrayList([]const u8) {
    const cwd = std.Io.Dir.cwd();
    var dir = try cwd.openDir(io, basePath, .{ .iterate = true });
    defer dir.close(io);
    ...
    while (try walker.next(io)) |entry| {
        if (entry.kind == .file and std.mem.eql(u8, std.fs.path.extension(entry.basename), ".proto")) {
            // Build full path by joining basePath and entry.path
            const path = try std.fs.path.join(allocator, &[_][]const u8{ basePath, entry.path });
