...
pub const QuicEngine = struct {
    ...
    pub fn init(self: *QuicEngine, allocator: Allocator, socket: UDP, is_client: bool) !void {
        ...
        const engine_api: c.lsquic_engine_api = .{ .ea_settings = &engine_settings, .ea_stream_if = &stream_if, .ea_stream_if_ctx = self, .ea_packets_out = packetsOut, .ea_packets_out_ctx = self, .ea_get_ssl_ctx = getSslContext };
...
fn onNewConn(ctx: ?*anyopaque, conn: ?*c.lsquic_conn_t) callconv(.c) ?*c.lsquic_conn_ctx_t {
    const engine: *QuicEngine = @ptrCast(@alignCast(ctx.?));
    ...
    const conn_ctx: *c.lsquic_conn_ctx_t = @ptrCast(@alignCast(lsquic_conn));
    c.lsquic_conn_set_ctx(conn, conn_ctx);
    if (!engine.is_client) {
        onHskDone(conn, c.LSQ_HSK_OK);
    }
...
fn onHskDone(conn: ?*c.lsquic_conn_t, status: c.enum_lsquic_hsk_status) callconv(.c) void {
    _ = conn;
    _ = status;
}

fn onConnClosed(conn: ?*c.lsquic_conn_t) callconv(.c) void {
    const lsquic_conn: *QuicConnection = @ptrCast(@alignCast(c.lsquic_conn_get_ctx(conn.?)));
    c.lsquic_conn_set_ctx(conn, null);
    lsquic_conn.engine.allocator.destroy(lsquic_conn);
    std.debug.print("Connection closed: {any}\n", .{conn});
}

fn onNewStream(ctx: ?*anyopaque, stream: ?*c.lsquic_stream_t) callconv(.c) ?*c.lsquic_stream_ctx_t {
    const engine: *QuicEngine = @ptrCast(@alignCast(ctx.?));
    const conn: *QuicConnection = @ptrCast(@alignCast(c.lsquic_conn_get_ctx(c.lsquic_stream_conn(stream.?))));
    ...
    const stream_ctx: *c.lsquic_stream_ctx_t = @ptrCast(@alignCast(lsquic_stream)); // Handle new stream logic here
...
fn onRead(
    stream: ?*c.lsquic_stream_t,
    stream_ctx: ?*c.lsquic_stream_ctx_t,
) callconv(.c) void {
    _ = stream;
    _ = stream_ctx;
}
...
test "lsquic engine initialization" {
    ...
    std.debug.print("LsquicEngine {} initialized successfully.\n", .{engine});
