...
const stream_if: lsquic_zig.StreamIf = .{
...
pub const QuicEngine = struct {
    ...
    pub fn init(self: *QuicEngine, allocator: Allocator, socket: UDP, transport: *QuicTransport, is_client_mode: bool) !void {
        var flags: c_uint = 0;
        if (!is_client_mode) {
            flags |= lsquic_zig.EngineFlags.SERVER;
        }

        var engine_settings: lsquic_zig.EngineSettings = undefined;
        lsquic_zig.initSettings(&engine_settings, flags);
        ...
        if (!lsquic_zig.checkSettings(&engine_settings, flags, &err_buf)) {
            std.log.warn("lsquic_engine_check_settings failed: {any}", .{err_buf});
            return error.InitializationFailed;
        }

        const ssl_ctx_fn: *const fn (?*anyopaque, ?*const anyopaque) callconv(.c) ?*anyopaque = @ptrCast(&getSslContext);
        const engine_api = lsquic_zig.EngineApi.init(
            &engine_settings,
            &stream_if,
            self,
            @ptrCast(&packetsOut),
        ...
        const engine = lsquic_zig.Engine.new(flags, &engine_api) catch {
            return error.InitializationFailed;
        };
    ...
    pub fn doConnect(self: *QuicEngine, peer_address: Multiaddr, callback_ctx: ?*anyopaque, callback: *const fn (ctx: ?*anyopaque, res: anyerror!*QuicConnection) void) void {
        ...
        const local_addr_ptr: *const lsquic_zig.SockAddr = @ptrCast(@alignCast(&self.local_address.any));
        const remote_addr_ptr: *const lsquic_zig.SockAddr = @ptrCast(@alignCast(&addrAndPeerId.address.any));
        std.log.debug(
            "lsquic_engine_connect local={any} remote={any}\n",
            .{ self.local_address.any, addrAndPeerId.address.any },
        );
        const conn_res = self.engine.connect(
            lsquic_zig.LsquicVersion.N_LSQVER,
            local_addr_ptr,
            remote_addr_ptr,
            self,
            null,
    ...
    fn processConns(self: *QuicEngine) void {
        ...
        if (self.engine.earliestAdvTick(&diff_us) > 0) {
            ...
            const granularity_us: u64 = @as(u64, lsquic_zig.LSQUIC_DF_CLOCK_GRANULARITY);
    ...
    fn getSslContext(
        peer_ctx: ?*anyopaque,
        _: ?*const lsquic_zig.SockAddr,
    ) callconv(.c) ?*lsquic_zig.SslCtxSt {
        const self: *QuicEngine = @ptrCast(@alignCast(peer_ctx.?));
        const res: *lsquic_zig.SslCtxSt = @ptrCast(@alignCast(self.ssl_context));
        return res;
    }
...
pub const QuicStream = struct {
    ...
    pub fn init(self: *QuicStream, stream: *lsquic_zig.Stream, conn: *QuicConnection) void {
        self.* = .{
            .stream = stream,
            .conn = conn,
...
fn onNewConn(ctx: ?*anyopaque, conn: ?*lsquic_zig.Connection) callconv(.c) ?*lsquic_zig.ConnectionContext {
    const engine: *QuicEngine = @ptrCast(@alignCast(ctx.?));
    ...
    const conn_ctx: *lsquic_zig.ConnectionContext = @ptrCast(@alignCast(lsquic_conn));
    conn.?.setContext(conn_ctx);

    // Server side will not call onHskDone, so we need to call it manually.
    if (!engine.is_client_mode) {
        onHskDone(conn, lsquic_zig.LSQ_HSK_OK);
    }

    return conn_ctx;
}

fn onHskDone(conn: ?*lsquic_zig.Connection, status: lsquic_zig.HskStatus) callconv(.c) void {
    if (status != lsquic_zig.LSQ_HSK_OK and status != lsquic_zig.LSQ_HSK_RESUMED_OK) {
        conn.?.close();
        return;
    } else {
...
pub fn onConnClosed(conn: ?*lsquic_zig.Connection) callconv(.c) void {
    const conn_raw: ?*lsquic.lsquic_conn_t = @ptrCast(conn);
    const lsquic_conn: *QuicConnection = @ptrCast(@alignCast(lsquic.lsquic_conn_get_ctx(conn_raw.?)));
    ...
    lsquic.lsquic_conn_set_ctx(conn_raw, null);
    lsquic_conn.engine.allocator.destroy(lsquic_conn);
}

fn onNewStream(ctx: ?*anyopaque, stream: ?*lsquic_zig.Stream) callconv(.c) ?*lsquic_zig.StreamContext {
    const stream_raw: ?*lsquic.lsquic_stream_t = @ptrCast(stream);
    const engine: *QuicEngine = @ptrCast(@alignCast(ctx.?));
    const conn: *QuicConnection = @ptrCast(@alignCast(lsquic.lsquic_conn_get_ctx(lsquic.lsquic_stream_conn(stream_raw.?))));
    const lsquic_stream: *QuicStream = engine.allocator.create(QuicStream) catch unreachable;
    lsquic_stream.init(stream.?, conn);

    const stream_ctx: *lsquic_zig.StreamContext = @ptrCast(@alignCast(lsquic_stream));
...
fn onStreamRead(
    stream: ?*lsquic_zig.Stream,
    stream_ctx: ?*lsquic_zig.StreamContext,
) callconv(.c) void {
    const self: *QuicStream = @ptrCast(@alignCast(stream_ctx.?));
    const s: *lsquic.lsquic_stream_t = @ptrCast(stream.?);
...
pub fn onStreamWrite(
    ...
    if (self.active_write) |*active_req| {
        const n_written = lsquic.lsquic_stream_write(stream_raw.?, active_req.data.items.ptr, active_req.data.items.len);
        ...
        if (n_written < 0) {
            ...
            if (err == posix.E.AGAIN) {
                ...
                _ = lsquic.lsquic_stream_wantwrite(stream_raw.?, 1);
                return;
            }
            ...
            _ = lsquic.lsquic_stream_flush(stream_raw.?);
        ...
        _ = lsquic.lsquic_stream_wantwrite(stream_raw.?, 0);
        return;
    }
}
