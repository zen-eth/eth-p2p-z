    ...
    fn drainDecrypted(self: *Self, ctx: *p2p_conn.ConnHandlerContext) !void {
        ...
        while (true) {
            ...
            if (n <= 0) {
                ...
                std.log.warn("tls_tcp: SSL_read error {} n={}", .{ err, n });
                // Drain the OpenSSL error queue so we know what actually failed.
                while (true) {
                    const code = ssl.ERR_get_error();
                    if (code == 0) break;
                    var buf: [256]u8 = undefined;
                    _ = ssl.ERR_error_string_n(code, &buf, buf.len);
                    const len = std.mem.indexOfScalar(u8, &buf, 0) orelse buf.len;
                    std.log.warn("  openssl err: {s}", .{buf[0..len]});
                }
                break;
            }
    ...
    pub fn onActive(self: *Self, ctx: *p2p_conn.ConnHandlerContext) !void {
        ...
        if (ret == 1) {
            ...
            if (err == ssl.SSL_ERROR_WANT_READ) {
                ...
                // Notify the upgrade caller before erroring up the pipeline,
                // otherwise the dialer waits forever on its completion event.
                self.callback(self.user_data, error.TlsHandshakeFailed);
        ...
        if (!self.handshake_done) {
            ...
            if (ret == 1) {
                ...
                if (err == ssl.SSL_ERROR_WANT_READ) {
                    ...
                    // Notify the upgrade caller so the dialer/listener doesn't
                    // wait forever on its completion event.
                    self.callback(self.user_data, error.TlsHandshakeFailed);
                    return error.TlsHandshakeFailed;
                }
            }
