...
pub fn enable() void {
    // `quiche_enable_debug_logging` installs a process-global callback and must not be
    // called twice; `swap` returns the prior value so only the first caller proceeds, and
    // `.acq_rel` makes the winner's swap synchronize with later observers.
