...
pub const DiscardResponder = struct {
    ...
    pub fn onMessage(self: *Self, _: *quic.QuicStream, msg: []const u8) anyerror!void {
        std.debug.print("Discard protocol responder received message: {s}\n", .{msg});
...
pub const DiscardSender = struct {
    ...
    pub fn send(self: *Self, message: []const u8, callback_ctx: ?*anyopaque, callback: *const fn (ctx: ?*anyopaque, res: anyerror!usize) void) void {
        std.debug.print("DiscardSender sending message: {s}\n", .{message});
...
test "discard protocol using switch" {
    ...
    defer {
        switch1.deinit();
        std.time.sleep(200 * std.time.ns_per_ms); // Give some time for cleanup before loop deinit
    }
    ...
    std.time.sleep(200 * std.time.ns_per_ms); // Wait for the stream to be established
...
//
//     defer {
//         std.debug.print("Starting unified test cleanup...\n", .{});
//         // 1. Deinit switches. This starts the graceful shutdown of connections
//         //    and the underlying transport engines.
//         switch1.deinit();
//         switch2.deinit();
//
//         // 2. IMPORTANT: Give time for the asynchronous close operations to complete.
//         //    The deinit call above is non-blocking. The event loops need time
//         //    to process the close events for all connections and streams.
//         std.time.sleep(500 * std.time.ns_per_ms);
//
//         // 3. Now that connections are closed, it's safe to stop the event loops.
//         loop.deinit();
//         cl_loop.deinit();
//         std.debug.print("Unified test cleanup finished.\n", .{});
//     }
//
